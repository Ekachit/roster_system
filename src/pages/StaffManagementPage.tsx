...
export function StaffManagementPage() {
  ...
  const load = useCallback(async () => {
    ...
    const loadError = queryError ?? locationsResult.error ?? activitiesResult.error
  ...
  async function save(event: FormEvent) {
    ...
    if (eligibilityLoading || eligibilityLoadError) {
      setError('Eligibility must load successfully before this staff profile can be saved.')
      return
    }
    const { error: saveError } = await supabase.rpc('save_staff_configuration', {
      ...
      p_activity_type_ids: activityIds,
    })
  ...
  async function beginEdit(row: StaffRecord) {
    ...
    const loadError = locationResult.error ?? activityResult.error
    if (loadError) {
      setEligibilityLoadError('Eligibility could not be loaded. Saving is disabled to protect existing eligibility.')
