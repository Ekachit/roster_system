...
import type { ReferenceRecord, ShiftAssignment, ShiftRecord, StaffRecord } from '../lib/types'
...
const blankForm = () => ({
  local_date: todayMelbourne(), start_time: '09:00', end_time: '17:00',
...
export function RosterPage() {
  ...
  function edit(shift: ShiftRecord) {
    setEditing(shift.id)
    setForm({
      local_date: shift.local_date, start_time: shift.start_time.slice(0, 5),
  ...
  return <div>
    ...
    <p className="mt-2 text-slate-600">Monday–Friday roster. All dates and times use {ROSTER_TIME_ZONE}.</p>
    ...
    {loading ? <div className="mt-6"><LoadingState /></div> : <div className="mt-6 grid gap-4 lg:grid-cols-5">
      {days.map((day) => <section key={day} aria-labelledby={`day-${day}`}>
        ...
        <div className="grid gap-3">{filtered.filter((item) => item.local_date === day).map((shift) => {
          const assigned = activeAssignments(shift.id)
          const understaffed = assigned.length < shift.required_staff_count
          return <article className={`card p-4 ${understaffed && shift.status !== 'CANCELLED' ? 'border-amber-400' : ''}`} key={shift.id}>
            ...
            <p className={`mt-2 text-sm font-medium ${understaffed ? 'text-amber-800' : 'text-green-800'}`}>{assigned.length}/{shift.required_staff_count} assigned {understaffed && '· Understaffed'}</p>
    ...
    {selectedShift && <section className="card mt-8" aria-labelledby="manage-heading">
      ...
      {activeAssignments(selectedShift.id).length === 0 ? <EmptyState title="No employees assigned" /> : <ul className="mt-2 grid gap-2">{activeAssignments(selectedShift.id).map((item) => <li className="flex flex-wrap items-center justify-between gap-2 rounded-lg border p-3" key={item.id}><span>{name(item.staff_id)}</span><span className="flex gap-2"><button className="button-secondary" onClick={() => void replace(item)}>Replace</button><button className="button-secondary" onClick={() => void remove(item)}>Remove</button></span></li>)}</ul>}
      <h3 className="mt-6 font-semibold">Candidates</h3>
      <ul className="mt-2 grid gap-2 sm:grid-cols-2">{candidates.map((candidate) => <li className="rounded-lg border p-3" key={candidate.staff_id}><div className="flex justify-between gap-2"><strong>{candidate.full_name}</strong><button className="button-secondary" disabled={candidate.conflicts.some((item) => !item.overridable)} onClick={() => void assign(candidate)}>Assign</button></div>{candidate.conflicts.length === 0 ? <p className="mt-1 text-sm text-green-800">Eligible and fully available</p> : <ul className="mt-2 list-disc pl-5 text-sm text-amber-900">{candidate.conflicts.map((item) => <li key={item.code}><strong>{item.code}</strong>: {item.message}{!item.overridable && ' (cannot override)'}</li>)}</ul>}</li>)}</ul>
    </section>}
  </div>
}
