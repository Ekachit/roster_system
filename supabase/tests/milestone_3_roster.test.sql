...
select lives_ok($$
  insert into roster_ids values('shift1',public.save_shift(null,'2026-08-03','10:00','13:00',
...
select throws_ok($$select public.save_shift(null,'2026-08-03','13:00','10:00',
...
select lives_ok($$
  insert into roster_ids values('assignment1',public.assign_employee((select id from roster_ids where key='shift1'),
    '22000000-0000-0000-0000-000000000002',false,null))
$$,'fully available employee is assigned');
select throws_like($$select public.assign_employee((select id from roster_ids where key='shift1'),'22000000-0000-0000-0000-000000000002',true,'duplicate')$$,
  'Assignment has a non-overridable conflict:%','duplicate assignment cannot be overridden');

select lives_ok($$
  insert into roster_ids values('shift2',public.save_shift(null,'2026-08-03','12:00','14:00',
    (select id from public.locations where name='Clayton'),(select id from public.activity_types where name='Training'),1,null))
$$,'second shift is created');
select throws_like($$select public.assign_employee((select id from roster_ids where key='shift2'),'22000000-0000-0000-0000-000000000002',true,'overlap')$$,
  'Assignment has a non-overridable conflict:%','overlap cannot be overridden');
select throws_ok($$select public.assign_employee((select id from roster_ids where key='shift1'),'22000000-0000-0000-0000-000000000003',false,null)$$,
...
select lives_ok($$
  insert into roster_ids values('assignment2',public.assign_employee((select id from roster_ids where key='shift1'),
    '22000000-0000-0000-0000-000000000003',true,'Supervisor accepts partial coverage for training'))
...
select lives_ok($$select public.replace_employee((select id from roster_ids where key='assignment2'),
  '22000000-0000-0000-0000-000000000004',true,'Exception reviewed with employee','Coverage changed')$$,
...
select throws_like($$select public.save_shift(null,'2026-08-03','09:00','10:00',
