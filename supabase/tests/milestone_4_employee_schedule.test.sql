...
select is((select count(*) from public.employee_schedule()),1::bigint,'employee sees own active published assignment only');
select is((select shift_title from public.employee_schedule()),'Synthetic published shift','employee sees published shift details');
select is((select count(*) from public.shifts),1::bigint,'shift RLS exposes only the employee assigned published shift');
select is((select count(*) from public.shift_assignments),1::bigint,'assignment RLS exposes only the employee own active published assignment');
...
select is((select count(*) from public.employee_schedule() where shift_title='Synthetic cancelled shift'),0::bigint,'cancelled assignment is not active in employee schedule');
...
select is((select colleague_names from public.employee_schedule()),array['Synthetic Employee Two']::text[],'co-worker response contains the assigned colleague name');
